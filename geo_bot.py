...
user_location = (53.902221, 27.561924)  # положение user по умолчанию (Центр Минска)
...
class UserPoint:
    def __init__(self, lon=53.902221, lat=27.561924):  # по умолчанию минск
        self.lon = lon
        self.lat = lat

    def coord(self):  # формат координат строка
        lon, lat = str(self.lon), str(self.lat)
...
#         self.user = MSQ.coord()
#         self.place = ANY.coord()
#         print('пользователь {} + место {}'.format(self.user, self.place))
#         return str('пользователь {} + место {}'.format(self.user, self.place))
#
#
# MSQ = UserPoint()
# PRK = UserPoint(message1[0], message1[1])
# print(f'MSQ: {MSQ.coord()}')
# print(f'PRK: {PRK.coord()}')
#
# ANY = AnyPoint()
# print(f'ANY coord: {ANY.coord()}')
#
# DIST = Distation()
# print(f'Distance: {DIST.calc_dis(MSQ, ANY)}')
#
#
# @bot.message_handler(content_types=['text'])
# def get_data(message):
